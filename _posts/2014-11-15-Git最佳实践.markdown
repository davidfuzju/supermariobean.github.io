...
我用`Git`的主要原因有以下三点：
  ...
  + 强大的功能集（这个说的有点泛，但似乎也找不到比较好的情况了）

最初来公司，公司整体的版本控制环境是`SVN`和`.NET`集成开发环境中的`TFS`。`SVN`是没有本地版本库的，无法对自己的提交作进一步的修改和安排，另外加之重量级`branch`导致我根本没法自由的创建本地分支并做一些处理；`TFS`集成在`.NET`的开发环境之下，我本身是`iOS`开发者，在`Mac OS X`上工作，所以基本上没有使用的可能。所以我一直都是开发团队中的叛逆者，当所有人都在已有的环境中开发时，我在寻找各种可以使用Git的方法和契机。

值得开心的是，先驱者们已经为我做了足够的探索，在实际的开发中我分别使用`git svn`和`git tf`作为`SVN`和`TFS`的前端，通过这两个命令与后端交互，前端仍旧可以使用`Git`的其他常用命令。

所以，本文会首先介绍`git svn`和`git tf`与后端的基本操作，基本操作主要包括：

  + 初始化一个本地代码库、克隆一个远程代码库到本地 `git init``git clone`

  + 将本地代码库中提交推送到远程代码库 `git push`

  + 将远程代码库更新到本地代码库 `git pull`

然后会介绍一些我自己在工作中常用的`Git`命令及其使用场景，另外也包括一些命令使用的原则和约定。
